...
apt install aptitude tmux git gcc make zsh kodi python3-virtualenv virtualenv vim cec-utils libcec-dev python3-cec scons swig snapclient libttspico-utils sshfs jq golang mosquitto-clients netcat-traditional mpv mpc liblgpio1 liblgpio-dev linux-headers device-tree-compiler raspi-utils || exit 
